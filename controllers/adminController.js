...
exports.pagesList = async (req, res) => {
  try {
    const episode = await Episode.findOne({ id: req.params.episode_id }).lean();
    ...
    const comic = await Comic.findOne({ id: episode.comic_id }).lean();
    ...
    console.log("PAGES LIST ERROR:", error);
    req.session.error = "Failed to load pages";
    res.redirect("/admin/comics");
  }
};
...
exports.deletePage = async (req, res) => {
  try {
    ...
    req.session.success = "Page deleted successfully";

    res.redirect(`/admin/pages/${page.episode_id}`);
  } catch (error) {
    console.log("DELETE PAGE ERROR:", error);

    req.session.error = "Failed to delete page";
    res.redirect("/admin/comics");
  }
};